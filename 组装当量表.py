...
读: 当量-段表.npz      F[p,a,b,n] (31,30,30,31) 原始段当量 S(p,a,b,n) ms
    ...
    错误成本-实测值.txt  错误成本标量 cost_ms (2026-09-06 起替代固定 500ms 常数, 用户决策;
...
写: 当量-修正段表.npz  C[p,a,b,n] = S(p,a,b,n) + cost_ms × P_err(a,b | sig(p), sig(n))
    （错误项按段 (p,n) 签名取列: p≠∅=有前键, n≠∅=有后键, 与 S 同语义;
      cost_ms = 错误成本实测标量 (净成本: 首错按下→首错位打对), 见 README §4.6/§4.7）
...
# 错误成本 = 实测标量 (09-06 起动态来源; 09-14 曾部署分手版, 09-15 用户决策回退——
#   手效应是退格位依赖的布局结构 (BS=CapsLock→左目标同手串行惩罚), 入表牺牲通用性;
#   txt 为 label 行式, 手别行保留为诊断, 组装只读 cost_ms 行)
try:
    _cost = {}
    for l in (PROJ / "错误成本-实测值.txt").read_text(encoding="utf-8").splitlines():
...
print(f"键对错误率: {len(perr)} 对 × 4 签名类 (错误损失实测标量 {ERR_MS:.1f}ms, n={ERR_N})  类均值: "
      f"角点 {Pmat[0,0].mean()*100:.2f}% / 首段 {Pmat[0,1].mean()*100:.2f}% / "
      f"尾段 {Pmat[1,0].mean()*100:.2f}% / 中段 {Pmat[1,1].mean()*100:.2f}%")

# ── 修正段表: +手别cost×P_err(a,b|签名), 按段 (p,n) 签名取列 ──
...
# 修正段表: +实测标量cost×P_err(a,b|签名), 按段 (p,n) 签名取列 (2026-09-15 回退标量——
# 09-14 分手部署经用户决策撤销: 手效应 (+69ms, t=4.15) 是退格位依赖的布局结构, 保当量表
# 对任意退格位的通用性; 手别系数保留在 错误成本-实测值.txt 作诊断, 不参与组装)
C = F + ERR_MS * Pexp
np.savez_compressed(PROJ / "当量-修正段表.npz", F=C,
                    ...
                    note=np.array(f"修正段当量 C(p,a,b,n)=S(p,a,b,n)+{ERR_MS:.1f}×P_err(a,b|有无前键p,有无后键n) ms "
                                  "(cost=错误成本实测标量, 来源 错误成本-实测值.txt; 09-14 分手版次日回退——"
                                  "手效应为退格位依赖结构, 保通用性); "
                                  "T2=C[30,a,b,30] T3=C[30,a,b,c]+C[a,b,c,30] "
                                  "T4=C[30,a,b,c]+C[a,b,c,d]+C[b,c,d,30]"))
print(f"输出: 当量-修正段表.npz  (C{C.shape}, 修正 = 段 + {ERR_MS:.1f}×签名错误率 (实测标量 n={ERR_N}), version 3)")
...
with open(PROJ / "当量-2-4键.txt", "w", encoding="utf-8") as f:
    f.write(f"# 2-4 键位当量 (ms, 期望耗时原值含错误成本: 修正段 = 段 + {ERR_MS:.1f}ms×P_err(a,b|有无前键,有无后键), 实测错误成本标量; 09-14 分手版 09-15 回退保通用性)\n")
