...
# 试次类型比例 (2键:3键:4键)。
# 历史: 设计比例 2:1:4 (09-19 设想) → 3 键补采样战役 1:7:2 (09-18~09-21) →
#   09-21 首次按瓶颈重定 3:4.5:2.5 → **同日去掉错误率模型交互项 γ 后按实测再定 3:5:2**。
# 现比例依据 (README §5.7「数据充分性」实测):
#   · 3 键 50%: T₃ 族有固定欠账 —— +1963 三键 trial 一次结清 T₃ 偏差判定/±2ms 带宽/c2 格 SE
#     (50% 份额 7.9 场); 3 键同时也是 3 键角色片的训练来源。
#   · 4 键 30%: 错误精度最优货币 —— 每 trial 错误事件 6.12% vs 3 键 2.55%、2 键 0.41%
#     (2.4× / 14.8×); 且去 γ 后 c4 长尾格成为「有后键 β」的约束项 (去掉它 SE ×1.51)。
#     主指标监控与 4 键角色片也靠它, 但不必回到设计比例的 57%。
#   · 2 键 20%: 只需维持角点片与 T₂ 计时监控 —— **c0 角点格已不再是 β 的锚**
#     (加法形式实测: 去掉 c0 只让 β 的 SE ×1.18、点估计仍 +0.341; β 已 z=3.5),
#     至此 2 键对 β 的杠杆很小, 份额不再由它决定。
P2, P3 = 0.20, 0.50  # 2键 20% / 3键 50% / 其余 4键 30%
